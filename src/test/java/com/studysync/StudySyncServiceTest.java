...
class StudySyncServiceTest {
    ...
    void completedAssignmentsExcludePendingAssignments() {
        Course course = service.createCourse("Algorithms", "CSCI 3320");
        Assignment completed = service.createAssignment(
                course.getId(), "Finished Project", "Implementation complete",
                LocalDateTime.now().plusDays(2), Assignment.Priority.HIGH);
        service.createAssignment(
                course.getId(), "Pending Project", "Still working",
                LocalDateTime.now().plusDays(1), Assignment.Priority.MEDIUM);
        service.completeAssignment(completed.getId());

        List<Assignment> assignments = service.getCompletedAssignments();
    ...
    void overdueAssignmentsOnlyReturnOverduePendingWork() {
        ...
        service.createAssignment(
                course.getId(), "Overdue Homework", "",
                LocalDateTime.now().minusDays(2), Assignment.Priority.HIGH);
        service.createAssignment(
                course.getId(), "Future Homework", "",
    ...
    void assignmentsCanBeFilteredByPriority() {
        ...
        service.createAssignment(
                course.getId(), "Low Priority", "",
                LocalDateTime.now().plusDays(3), Assignment.Priority.LOW);
        service.createAssignment(
                course.getId(), "High Priority Later", "",
                LocalDateTime.now().plusDays(2), Assignment.Priority.HIGH);
        service.createAssignment(
                course.getId(), "High Priority First", "",
                LocalDateTime.now().plusDays(1), Assignment.Priority.HIGH);

        List<Assignment> highPriority =
                service.getAssignmentsByPriority(Assignment.Priority.HIGH);
    ...
    void assignmentSearchMatchesTitleCaseInsensitively() {
        Course course = service.createCourse(
                "Computer Architecture", "CSCI 3212");
        service.createAssignment(
                course.getId(), "Pipeline Review", "Study processor stages",
                LocalDateTime.now().plusDays(1), Assignment.Priority.HIGH);
        service.createAssignment(
                course.getId(), "Memory Homework", "Cache hierarchy",
                LocalDateTime.now().plusDays(2), Assignment.Priority.MEDIUM);

        List<Assignment> results = service.searchAssignments("PIPELINE");
    ...
    void assignmentSearchMatchesDescriptionCaseInsensitively() {
        Course course = service.createCourse("Linear Algebra", "MATH 2502");
        service.createAssignment(
                course.getId(), "Homework 1", "Practice MATRIX operations",
                LocalDateTime.now().plusDays(1), Assignment.Priority.MEDIUM);
        service.createAssignment(
                course.getId(), "Homework 2", "Vector practice",
                LocalDateTime.now().plusDays(2), Assignment.Priority.LOW);

        List<Assignment> results = service.searchAssignments("matrix");
        assertEquals(1, results.size());
    ...
    @Test
    void updateAssignmentChangesStoredDetails() {
        Course originalCourse = service.createCourse(
                "Data Structures", "CSCI 3300");
        Course newCourse = service.createCourse(
                "Computer Architecture", "CSCI 3212");
        Assignment assignment = service.createAssignment(
                originalCourse.getId(), "Old Title", "Old description",
                LocalDateTime.now().plusDays(1), Assignment.Priority.LOW);
        LocalDateTime newDueDate = LocalDateTime.now().plusDays(5);

        assertTrue(service.updateAssignment(
                assignment.getId(), newCourse.getId(), "Updated Title",
                "Updated description", newDueDate, Assignment.Priority.HIGH));

        Assignment updated = service.getAssignments().get(0);
        assertEquals(newCourse.getId(), updated.getCourseId());
    ...
    @Test
    void updateAssignmentPreservesCompletionStatus() {
        Course course = service.createCourse("Data Structures", "CSCI 3300");
        Assignment assignment = service.createAssignment(
                course.getId(), "Homework", "",
                LocalDateTime.now().plusDays(1), Assignment.Priority.MEDIUM);
        service.completeAssignment(assignment.getId());

        service.updateAssignment(
                assignment.getId(), course.getId(), "Revised Homework",
                "Updated after completion", LocalDateTime.now().plusDays(2),
                Assignment.Priority.HIGH);

        Assignment updated = service.getCompletedAssignments().get(0);
        assertTrue(updated.isCompleted());
        assertEquals("Revised Homework", updated.getTitle());
    }

    @Test
    void updateAssignmentRejectsMissingAssignment() {
        Course course = service.createCourse("Data Structures", "CSCI 3300");

        assertThrows(IllegalArgumentException.class,
                () -> service.updateAssignment(
                        999, course.getId(), "Homework", "",
                        LocalDateTime.now().plusDays(1), Assignment.Priority.HIGH));
    }

    @Test
    void updateAssignmentRejectsMissingDestinationCourse() {
        Course course = service.createCourse("Data Structures", "CSCI 3300");
        Assignment assignment = service.createAssignment(
                course.getId(), "Homework", "",
                LocalDateTime.now().plusDays(1), Assignment.Priority.HIGH);
        ...
        assertThrows(IllegalArgumentException.class,
                () -> service.updateAssignment(
                        assignment.getId(), 999, "Homework", "",
                        LocalDateTime.now().plusDays(2), Assignment.Priority.MEDIUM));
    }

    @Test
    void updateAssignmentRejectsInvalidFields() {
        Course course = service.createCourse("Data Structures", "CSCI 3300");
        Assignment assignment = service.createAssignment(
                course.getId(), "Homework", "",
                LocalDateTime.now().plusDays(1), Assignment.Priority.HIGH);
        ...
        assertThrows(IllegalArgumentException.class,
                () -> service.updateAssignment(
                        assignment.getId(), course.getId(), "Homework", "",
                        null, Assignment.Priority.MEDIUM));
        assertThrows(IllegalArgumentException.class,
                () -> service.updateAssignment(
                        assignment.getId(), course.getId(), "Homework", "",
    ...
    @Test
    void deleteAssignmentRemovesStoredAssignment() {
        Course course = service.createCourse("Data Structures", "CSCI 3300");
        Assignment assignment = service.createAssignment(
                course.getId(), "Delete Me", "",
                LocalDateTime.now().plusDays(1), Assignment.Priority.LOW);

        assertTrue(service.deleteAssignment(assignment.getId()));
        assertTrue(service.getAssignments().isEmpty());
    }

    @Test
    void deleteAssignmentRejectsMissingAssignment() {
        assertThrows(IllegalArgumentException.class,
                () -> service.deleteAssignment(999));
    }
    ...
    void recordStudySessionRequiresExistingCourse() {
        assertThrows(IllegalArgumentException.class,
                () -> service.recordStudySession(
                        999, LocalDateTime.now(), 60, "Review"));
    }
    ...
    void totalStudyMinutesAreCalculated() {
        Course course = service.createCourse("Data Structures", "CSCI 3300");
        service.recordStudySession(
                course.getId(), LocalDateTime.now(), 60, "Trees");
        service.recordStudySession(
                course.getId(), LocalDateTime.now().plusHours(2), 45, "Graphs");
    ...
    void completionPercentageIsZeroWithoutAssignments() {
        assertEquals(0.0,
                service.getAssignmentCompletionPercentage(), 0.001);
    }
    ...
    void completionPercentageReflectsCompletedAssignments() {
        ...
        Assignment first = service.createAssignment(
                course.getId(), "Homework 1", "",
                LocalDateTime.now().plusDays(1), Assignment.Priority.HIGH);
        service.createAssignment(
                course.getId(), "Homework 2", "",
                LocalDateTime.now().plusDays(2), Assignment.Priority.MEDIUM);
    ...
    void dashboardSummaryReflectsAcademicProgress() {
        Course firstCourse = service.createCourse(
                "Data Structures", "CSCI 3300");
        service.createCourse("Linear Algebra", "MATH 2502");
        Assignment completed = service.createAssignment(
                firstCourse.getId(), "Completed Homework", "",
                LocalDateTime.now().plusDays(1), Assignment.Priority.MEDIUM);
        service.createAssignment(
                firstCourse.getId(), "Overdue Homework", "",
                LocalDateTime.now().minusDays(1), Assignment.Priority.HIGH);
        service.completeAssignment(completed.getId());
        service.recordStudySession(
                firstCourse.getId(), LocalDateTime.now(), 60, "Trees");
        service.recordStudySession(
                firstCourse.getId(), LocalDateTime.now().plusHours(2), 30, "Graphs");
